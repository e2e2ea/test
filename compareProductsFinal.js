      ...
      for (const ext of sub.childItems) {
        ...
        for (const data of woolworthsData) {
          ...
          if (filteredProducts && filteredProducts.length > 0) {
            if(filteredProducts[0].prices.length === 0) return
        ...
        try {
            ...
            try {
              ...
              if (unMatchedColes.length > 0) {
                ...
                if (fs.existsSync(filePath)) {
                  ...
                  const combinedData = [...data, ...unMatchedColes];
                  ...
                  if (unMatchedColes.length > 0) {
                    ...
                    fs.writeFileSync(filePath, JSON.stringify(unMatchedColes, null, 2));
                    console.log(`Data saved to ${filePath}`);
                  }
                }
                try {
                  ...
                  const response = await axios.post(externalApiUrl, unMatchedColes, {
                    headers: {
                 ...
                 console.log('error passing data:', `/${categ.id}/${ext.subId ?? ''}${ext.childId && ` - ${ext.childId}`}`)
                  if (error.response) {
          ...
          if (unmatchedWoolworthsProducts && unmatchedWoolworthsProducts.length > 0) {
            ...
            try {
              ...
              const response = await axios.post(externalApiUrl, unmatchedWoolworthsProducts, {
                headers: {
                  accept: 'application/json',
                  'X-API-Key': apiKey,
                  'Content-Type': 'application/json',
